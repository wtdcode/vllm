...
class ParserEngine(Parser):
    ...
    def _add_tool_call_stop(
        ...
        open another one, and in agentic workloads it does -- repeatedly,
    ...
    def is_reasoning_end_streaming(
        ...
        # O(len(delta)). The inherited default rescans the whole sequence,
        # and the structured-output manager calls this per request per engine
        # step -- and per speculative position -- while a thinking span is
        # open. With reasoning spans of tens of thousands of tokens that
        # full rescan serializes hundreds of milliseconds of Python into
        # every step and stalls the engine loop.
        end_id = self._reasoning_end_token_id
        if end_id is None:
